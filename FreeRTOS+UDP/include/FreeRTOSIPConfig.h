...
/* RPL - added to make sure the UDP data structures are properly handled */
#define  ipconfigBYTE_ORDER FREERTOS_LITTLE_ENDIAN

/* RPL - added to define the buffers to handle frames */
#define EMAC_ETH_MAX_FLEN 2000
...
state (so other tasks can continue to be executed) until either a network buffer
becomes available or the send block time expires.  If the send block time expires
then the send operation is aborted.  The maximum allowable send block time is
capped to the value set by ipconfigMAX_SEND_BLOCK_TIME_TICKS.  Capping the
maximum allowable send block time prevents a deadlock occurring when all the
network buffers are in use and the tasks that process (and subsequently free)
the network buffers are themselves blocked waiting for a network buffer.
...
#define ipconfigNUM_NETWORK_BUFFERS     48
